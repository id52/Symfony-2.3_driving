...
use My\AppBundle\Entity\ThemeReader;
...
class MyTrainingController extends MyAbstract
{
    ...
    public function init()
    {
        ...
        if (false === $cntxt->isGranted('ROLE_USER_PAID2')) {
            $limit = clone $this->user->getPayment1Paid();
            $limit->add(new \DateInterval('P'.$this->settings['access_time_after_1_payment'].'D'));

            if ($limit < new \DateTime()) {
                throw new AppResponseException($this->render('AppBundle:My:timelimit_after_1_payment.html.twig', array(
        ...
        if ($cntxt->isGranted('ROLE_USER_PAID2') && false === $cntxt->isGranted('ROLE_USER_FULL_PROFILE')) {
            throw new AppResponseException($this->render('AppBundle:My:profile_not_full.html.twig'));
        }
    }
    ...
    public function trainingsAction()
    {
        ...
        $settings = $this->em->getRepository('AppBundle:Setting')->getAllData();
        $message = '';
        if (isset($settings['training_final_exam_description'])) {
            $placeholders = array();
            $placeholders['{{ exam }}'] = count($subjects);
            $message = $settings['training_final_exam_description'];
        ...
        return $this->render('AppBundle:My:trainings.html.twig', array(
            ...
            'help_btn'       => $this->settingsNotifies['trainings_help_btn'],
            'training_final_exam_description' => $message,
        ));
    }
    ...
    public function trainingAction($id)
    {
        ...
        return $this->render('AppBundle:My:training.html.twig', array(
            ...
            'exam'     => $exam,
            'subject'  => $subject,
            'help_btn' => $this->settingsNotifies['training_'.$subject->getId().'_help_btn'],
        ));
    }
    ...
    public function trainingThemeAction($id)
    {
        ...
        foreach ($flash_blocks as $block) { /** @var $block \My\AppBundle\Entity\FlashBlock */
        ...
        return $this->render('AppBundle:My:training_theme.html.twig', array(
            'theme'            => $theme,
            'help_btn'         => $this->settingsNotifies['theme_help_btn'],
    ...
    {
        $themesRepo= $this->em->getRepository('AppBundle:Theme');

        $theme = $themesRepo->find($id);
        if (!$theme) {
            throw $this->createNotFoundException('Theme for id "'.$id.'" not found.');
        }
        ...
        $prev_theme = $themesRepo->createQueryBuilder('t')
            ->andWhere('t.position < :position')->setParameter(':position', $theme->getPosition())
            ->andWhere('t.subject = :subject')->setParameter(':subject', $theme->getSubject())
            ->leftJoin('t.versions', 'v')
            ->andWhere('v = :version')->setParameter(':version', $this->version)
            ->addOrderBy('t.position', 'DESC')
            ->setMaxResults(1)->getQuery()->getOneOrNullResult();
        if (!$themesRepo->isReaderExists($theme, $this->user)
            && (!$prev_theme || $themesRepo->isReaderExists($prev_theme, $this->user))
        ) {
            $questions_access = true;
        ...
        if (!$log) {
            $questions = $this->em->getRepository('AppBundle:Question')->getQuestionsIdsArray($theme, $this->version);
            shuffle($questions);
            ...
            if ($num > end($keys) || count($keys) == 0) {
                ...
                if (count($questions) > 0) {
                    ...
                    if (!$themesRepo->isReaderExists($theme, $this->user)) {
                        ...
                        $themeReader->setTheme($theme);
                        $themeReader->setReader($this->user);

                        $this->em->persist($themeReader);
                        $this->em->flush();
                    }
                ...
                $message = 'Question for number "'.$num.'" in theme for id "'.$id.'" not found.';
        ...
        if ($request->isMethod('post')) {
            ...
            if ($request->isXmlHttpRequest()) {
                ...
                $params = array(
                    ...
                    'help_btn'               => $this->settingsNotifies['question_help_btn'],
                );
    ...
    public function trainingThemeTestResetAction($id)
    {
        ...
        $themeReader = $this->em->getRepository('AppBundle:ThemeReader')
            ->findOneBy(['theme' => $theme, 'reader' => $this->user]);
        if ($themeReader) {
            $this->em->remove($themeReader);
            $this->em->flush();
        }
    ...
    public function trainingSliceAction(Request $request, $id)
    {
        ...
        if (!$log) {
            ...
            foreach ($themes as $theme) {
                ...
                $theme = $theme[0];
                $orig_questions[] = $this->em->getRepository('AppBundle:Question')
                    ->getQuestionsIdsArray($theme, $this->version);
            }
    ...
    {
        $access = $request->query->get('access', 0);
        ...
        if (!$log) {
            if ($this->resetAttempts($subject)) {
                return $this->redirect($this->generateUrl('my_training', ['id' => $subject->getId()]));
            }
            ...
            $retakeInterval = $this->settings['attempts_days_of_retake'];

            if ($last_log and !$last_log->getEndedAt() and $retakeInterval) {
                $this->checkAndSetPenaltyPeriod($last_log->getStartedAt());
            }

            $isFree = false;
            if ($this->user->getPenaltyPeriod() > new \DateTime()) {
                if ($this->user->getExamAttempts() == 0) {
                    $message      = $this->settingsNotifies['attempts_buy_text'];

                    $rem_time = $this->user->getPenaltyPeriod() ?
                        $this->user->getPenaltyPeriod()->diff(new \DateTime('now')) : null;
                    $seconds_left = Time::getAllSeconds($rem_time);

                    $placeholders = [
                        '{{ time_left }}' => '<span class="retake_countdown" data-seconds-left="'
                            .$seconds_left.'"></span>',
                        '{{ date }}'      => $this->user->getPenaltyPeriod()->format('H:m d.m.Y'),
                    ];

                    $message = str_replace(array_keys($placeholders), array_values($placeholders), $message);

                    return $this->render('AppBundle:My:training_exam_attempt_buy.html.twig', [
                        'subject' => $subject,
                        'message' => $message,
                    ]);
                } else {
                    if ($access == 0) {
                        $message = $this->settingsNotifies['second_attempt_text'];

                        /** @var $lastAttemptPackage \My\AppBundle\Entity\ExamAttemptLog */
                        $lastAttemptPackage = $this->em->getRepository('AppBundle:ExamAttemptLog')
                        ...
                        $rem_time = $this->user->getPenaltyPeriod()
                            ? $this->user->getPenaltyPeriod()->diff(new \DateTime('now'))
                            : null;
                        $seconds_left = Time::getAllSeconds($rem_time);
                        ...
                        $placeholders = [
                            ...
                            '{{ time_left }}' => '<span class="retake_countdown" data-seconds-left="'
                                .$seconds_left.'"></span>',
                            '{{ try }}'       => $this->user->getExamAttempts(),
                            '{{ buy }}'       => $lastAttemptPackage ? $lastAttemptPackage->getAmount() : 0,
                        ];

                        $message = str_replace(array_keys($placeholders), array_values($placeholders), $message);

                        return $this->render('AppBundle:My:training_exam_attempt_use.html.twig', [
                            'subject' => $subject,
                            'message' => $message,
                        ]);
                    }
                ...
                $isFree = true;
                if ($access == 0) {
                    $message      = $this->settingsNotifies['info_attempts_text'];
                    $placeholders = ['{{ days }}' => $this->settings['attempts_days_of_retake']];
                    $message      = str_replace(array_keys($placeholders), array_values($placeholders), $message);

                    return $this->render('AppBundle:My:training_exam_attempt_free.html.twig', [
                        'subject' => $subject,
                        'message' => $message,
                    ]);
                }
            ...
            if (!$isFree) {
                $this->user->reduceExamAttempts();
                $this->em->persist($this->user);
            }

            $examAttemptLog = new ExamAttemptLog();
            $examAttemptLog->setIsFree($isFree);
            $examAttemptLog->setAmount($isFree ? 0 : -1);
            $examAttemptLog->setUser($this->getUser());
            $examAttemptLog->setExamLog($log);
            $examAttemptLog->setSubject($subject);
            $this->em->persist($examAttemptLog);
        ...
        if ($s_data['l_activity'] < $activity_limit) {
            ...
            if ($request->isXmlHttpRequest()) {
                ...
                return $this->render('AppBundle:My:training_exam_longtime.html.twig', array('subject' => $subject));
            }
        }
        $s_data['l_activity'] = new \DateTime();
        $session->set($s_name, $s_data);
        ...
        if ($s_data['ticket_end_time'] && $s_data['ticket_end_time'] < new \DateTime()) {
            ...
            if ($request->isXmlHttpRequest()) {
                ...
                return $this->render('AppBundle:My:training_exam_timeout.html.twig', array('subject' => $subject));
            }
        }
    ...
    protected function examEnd($s_name, ExamLog $log, $is_passed = false)
    {
        ...
        $examAttemptLog = $this->em->getRepository('AppBundle:ExamAttemptLog')->findOneBy(['exam_log' => $log]);
        if ($examAttemptLog) {
            $examAttemptLog->setIsPassed($is_passed);
            $this->em->persist($examAttemptLog);
        }

        $this->em->flush();

        $this->resetAttempts($log->getSubject());
    ...
    public function trainingFinalExamAction(Request $request)
    {
        ...
        if (!$log) {
            ...
            $last_log = $this->em->getRepository('AppBundle:FinalExamLog')->createQueryBuilder('fel')
            ...
            if ($this->user->getPenaltyPeriod() > new \DateTime()) {
                if ($this->user->getExamAttempts() == 0) {
                    ...
                    return $this->render('AppBundle:My:training_final_exam_attempt_buy.html.twig', [
                        'message' => $message,
                    ]);
                } else {
                    if ($access == 0) {
                        ...
                        return $this->render('AppBundle:My:training_final_exam_attempt_use.html.twig', [
                            'message' => $message,
                        ]);
                    }
                }
                ...
                if ($access == 0) {
                    ...
                    return $this->render('AppBundle:My:training_final_exam_attempt_free.html.twig', [
                        'message' => $message,
                    ]);
                }
            }
...
//                ->leftJoin('t.subject', 's')->addSelect('s')
//                ->leftJoin('t.questions', 'q')->addSelect('q')
//                ->leftJoin('q.versions', 'qv')->addSelect('qv')
//                ->leftJoin('q.image', 'i')->addSelect('i')
                ->leftJoin('t.versions', 'v')
                ->andWhere('v = :version')->setParameter(':version', $this->version)
                ->getQuery()->execute();
            ...
            foreach ($themes as $theme) { /** @var $theme \My\AppBundle\Entity\Theme */
                $orig_questions[] = $this->em->getRepository('AppBundle:Question')
                    ->getQuestionsIdsArray($theme, $this->version, true);
            }
            ...
            $examAttemptLog->setAmount($isFree ? 0 : -1);
            $examAttemptLog->setUser($this->getUser());
            $examAttemptLog->setFinalExamLog($log);
            $this->em->persist($examAttemptLog);
    ...
    protected function finalExamEnd($s_name, FinalExamLog $log, $is_passed = false)
    {
        ...
        if ($is_passed) {
            ...
            $this->user->setUnsubscribedX(true);
            $this->em->persist($this->user);
        }

        $examAttemptLog = $this->em->getRepository('AppBundle:ExamAttemptLog')->findOneBy(['final_exam_log' => $log]);
        if ($examAttemptLog) {
            $examAttemptLog->setIsPassed($is_passed);
            $this->em->persist($examAttemptLog);
        }
    ...
    public function trainingFinalExamResetAction()
    {
        ...
        $this->user->setUnsubscribedX(false);
        $this->em->persist($this->user);
        $this->em->flush();
    ...
    {
        if (false === $this->get('security.context')->isGranted('ROLE_USER_PAID2')) {
            return $this->render('AppBundle:My:training_without_2_payment.html.twig');
        }

        return false;
    }

    protected function resetAttempts($subject = null)
    {
        $errorsCount = $this->get('app.exam_attempts');
        $errors = $errorsCount->getErrorsCountByUser($this->user);

        if ($errors >= $this->settings['attempts_to_reset']) {
            $exam_attempt_log = new ExamAttemptLog();
            $exam_attempt_log->setAmount(0);
            $exam_attempt_log->setUser($this->getUser());
            $exam_attempt_log->setSubject($subject);
            $this->em->persist($exam_attempt_log);
            ...
            $this->em->getRepository('AppBundle:SliceLog')->createQueryBuilder('sl')
                ...
                ->andWhere('sl.user = :user')->setParameter('user', $this->user)
                ->getQuery()->execute();
            $this->em->getRepository('AppBundle:ThemeReader')->createQueryBuilder('tr')
                ->delete()
                ->andWhere('tr.reader = :user')->setParameter('user', $this->user)
                ->getQuery()->execute();

            $this->em->flush();

            $this->get('session')->getFlashBag()->add('attempts_reset_progress', true);
            return true;
        }
        return false;
    }

    protected function checkAndSetPenaltyPeriod($retakeTime = null)
    {
        if (empty($retakeTime)) {
            $retakeTime = new \DateTime();
        }

        if ($this->user->getPenaltyPeriod() < new \DateTime()) {
            $retakeInterval = $this->settings['attempts_days_of_retake'];
            $retakeTime->add(new \DateInterval('P' . $retakeInterval . 'D'));

            $user = $this->getUser();
            /** @var $user \My\AppBundle\Entity\User */
            $user->setPenaltyPeriod($retakeTime);
