...
class SendMailingCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $cnt = 0;
        $testUsers = array();
        $testEmails = $em->getRepository('AppBundle:TestEmail')->findAll();
        foreach ($testEmails as $testEmail) {
            /** @var $testEmail \My\AppBundle\Entity\TestEmail */

            $testUsers[] = array(
                'email'      => $testEmail->getEmail(),
                'first_name' => $translator->trans('testEmail_first_name'),
                'last_name'  => $translator->trans('testEmail_last_name'),
            );
        }
        ...
        if ($mailing) {
            foreach ($mailing as $mail) {
                /** @var $mail \My\AppBundle\Entity\Mailing */
